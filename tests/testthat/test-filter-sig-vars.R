...
small_exp <- function() {
  test_gp_exp |>
    ...
    glyexp::mutate_obs(group = factor(.data$group, levels = c("H", "M", "Y", "C")))
}
...
test_that("filter_sig_vars works for ANOVA with p_val_cutoff", {
  ...
  sig_exp <- filter_sig_vars(exp, result, p_val_cutoff = 0.05, p_adj_cutoff = NULL)
  expect_equal(nrow(sig_exp), 5)
})
...
test_that("filter_sig_vars fails for ANOVA with comparison on main test", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, comparison = "C_vs_H"), error = TRUE)
})

test_that("filter_sig_vars fails for ANOVA with both p_val_cutoff and p_adj_cutoff", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, p_val_cutoff = 0.05, p_adj_cutoff = 0.05), error = TRUE)
})

test_that("filter_sig_vars fails for ANOVA with neither p_val_cutoff, p_adj_cutoff", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, p_val_cutoff = NULL, p_adj_cutoff = NULL), error = TRUE)
})
...
test_that("filter_sig_vars works for ANOVA on post-hoc results", {
  ...
  post_hoc_res <- result$tidy_result$post_hoc_test |>
    ...
    dplyr::mutate(p_adj = dplyr::if_else(.data$row_num %% 2 == 0 & .data$comparison == "H_vs_C", 0.01, 1))
  result$tidy_result$post_hoc_test <- post_hoc_res
  sig_exp <- filter_sig_vars(exp, result, on = "post_hoc_test", comparison = "H_vs_C")
...
test_that("filter_sig_vars fails for ANOVA on post-hoc results without comparison", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, on = "post_hoc_test"), error = TRUE)
})

test_that("filter_sig_vars fails for ANOVA with invalid comparison format", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, on = "post_hoc_test", comparison = "H_vs_C_vs_M"), error = TRUE)
})

test_that("filter_sig_vars fails for ANOVA with non-existent comparison", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, on = "post_hoc_test", comparison = "H_vs_D"), error = TRUE)
})

test_that("filter_sig_vars fails for ANOVA with comparison of wrong order", {
  exp <- small_exp()
  result <- suppressMessages(gly_anova(exp))
  expect_snapshot(filter_sig_vars(exp, result, on = "post_hoc_test", comparison = "C_vs_H"), error = TRUE)
})
