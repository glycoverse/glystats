...
For multifactor experiments, use a one-sided formula to describe the design.
Named contrasts can combine model coefficients; wrap interaction coefficient
names in backticks:
...
`subject_col`. `gly_ttest()` and `gly_wilcox()` then match the two groups by
subject, while `gly_anova()` and `gly_kruskal()` use repeated-measures ANOVA and
the Friedman test, respectively:
